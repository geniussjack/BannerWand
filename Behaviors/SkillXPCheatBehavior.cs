...
namespace BannerWand.Behaviors
{
    ...
    /// Pre-allocated collections are reused instead of creating new ones each tick.
    /// </para>
    /// </remarks>
    public class SkillXPCheatBehavior : CampaignBehaviorBase
    {
        ...
        /// Cleared and reused on each hourly tick.
        ...
        private static void ApplyUnlimitedSkillXP()
        {
            ...
            if (targetSettings.HasAnyNPCTargetEnabled())
            {
                ...
                // Filter heroes that should receive cheats
                ...
                // Apply XP to filtered heroes
                for (int i = 0; i < _heroPool.Count; i++)
                {
                    Hero hero = _heroPool[i];
        ...
        private static void ApplyTroopXPBoost()
        {
            ...
            if (targetSettings.HasAnyNPCTargetEnabled())
            {
                ...
                // Filter parties that should receive cheats
                ...
                // Apply XP to filtered parties
                for (int i = 0; i < _partyPool.Count; i++)
                {
                    MobileParty party = _partyPool[i];
