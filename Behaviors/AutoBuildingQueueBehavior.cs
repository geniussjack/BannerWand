        ...
        private void OnDailyTick()
        {
            try
            {
                ...
                foreach (Settlement settlement in Settlement.All)
                {
                    ...
                    // OPTIMIZED: Get all buildings that can be upgraded using for loop instead of LINQ
                    // (not at max level and not daily projects)
                    List<Building> availableBuildings = new(capacity: town.Buildings.Count);
                    for (int i = 0; i < town.Buildings.Count; i++)
                    {
                        Building building = town.Buildings[i];
