...
namespace BannerWandRetro.Models
{
    ...
    public class CustomSettlementSecurityModel : DefaultSettlementSecurityModel
    {
        ...
        public override ExplainedNumber CalculateSecurityChange(Town town, bool includeDescriptions = false)
        {
            ...
            {
                Error($"[CustomSettlementSecurityModel] Error in CalculateSecurityChange: {ex.Message}");
                ...
                {
                    Error($"[CustomSettlementSecurityModel] Fallback also failed: {fallbackEx.Message}");
