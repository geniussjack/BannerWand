...
{
    /// <summary>
    /// Central manager for coordinating all cheat functionalities in BannerWand.
    ...
    {
        #region Object Pooling - Performance Optimization

        /// <summary>
        /// Pooled list for party collection to avoid repeated allocations.
        /// Cleared and reused for each operation that needs party filtering.
        /// Expected capacity: ~200 parties in typical campaign.
        /// </summary>
        private static readonly List<MobileParty> _partyPool = new(capacity: 200);

        /// <summary>
        /// Pooled list for hero collection to avoid repeated allocations.
        /// Cleared and reused for each operation that needs hero filtering.
        /// Expected capacity: ~100 heroes in typical campaign.
        /// </summary>
        private static readonly List<Hero> _heroPool = new(capacity: 100);
        ...
        public static void Initialize(bool showMessage = true)
        {
            try
            {
                ...
                if (showMessage && !_initializationMessageShown)
                {
                    ...
                    _ = welcomeMessage.SetTextVariable("COUNT", activeCheatCount);
                ...
                using (ModLogger.BeginPerformanceScope($"Apply Gold ({amount})"))
                {
                    ...
                    if (affectedHeroCount > 0)
                    {
                        ...
                        if (affectedHeroCount == 1)
                        {
                            ...
                            _ = resultMessage.SetTextVariable("AMOUNT", amount);
                        }
                        else
                        {
                            resultMessage = new("{=BW_Message_GoldChangedMultiple}Gold changed by {AMOUNT} for {COUNT} heroes");
                            _ = resultMessage.SetTextVariable("AMOUNT", amount);
                            _ = resultMessage.SetTextVariable("COUNT", affectedHeroCount);
                        }
                ...
                using (ModLogger.BeginPerformanceScope($"Apply Influence ({amount})"))
                {
                    ...
                    if (affectedClanCount > 0)
                    {
                        ...
                        if (affectedClanCount == 1)
                        {
                            ...
                            _ = resultMessage.SetTextVariable("AMOUNT", (int)amount);
                        }
                        else
                        {
                            resultMessage = new("{=BW_Message_InfluenceChangedMultiple}Influence changed by {AMOUNT} for {COUNT} clans");
                            _ = resultMessage.SetTextVariable("AMOUNT", (int)amount);
                            _ = resultMessage.SetTextVariable("COUNT", affectedClanCount);
                        }
        ...
        /// </summary>
        /// <returns>A pooled list of <see cref="MobileParty"/> instances that match the target criteria.</returns>
        /// <remarks>
        /// <para>
        /// IMPORTANT: This method returns a POOLED list that is reused across calls.
        /// Do NOT store references to this list or modify it after the calling method completes.
        /// Copy the contents if you need to persist the data.
        /// </para>
        /// <para>
        /// Performance: O(n) where n is the number of mobile parties in the game.
        /// Uses pre-allocated list to avoid GC pressure from repeated allocations.
        /// </para>
        /// </remarks>
        public static List<MobileParty> GetAffectedParties()
        {
            // Clear pooled list for reuse
            _partyPool.Clear();
            ...
            {
                _partyPool.Add(MobileParty.MainParty);
            }

            // Add NPC parties if any NPC targets are enabled in settings
            if (targetSettings.HasAnyNPCTargetEnabled())
            {
                // Use cached collection instead of direct MobileParty.All enumeration
                ...
                for (int i = 0; i < allParties.Count; i++)
                {
                    ...
                    // Skip player's main party (already added)
            ...
            try
            {
                using (ModLogger.BeginPerformanceScope("Heal All Troops"))
                {
                    ...
                    if (totalHealedTroops > 0)
                    {
                        ...
                        _ = healMessage.SetTextVariable("COUNT", totalHealedTroops);
