...
namespace BannerWand.Patches
{
    ...
    public static class NavalSpeedPatch
    {
        ...
        /// <summary>
        /// Flag indicating whether type lookup has been completed (success or failure).
        /// Prevents repeated reflection attempts if type is not found.
        /// </summary>
        private static bool _typeCheckCompleted = false;

        /// <summary>
        /// Gets the NavalDLC model type, caching it for performance.
        /// Uses a completion flag to avoid repeated reflection attempts if type is not found.
        /// </summary>
        private static Type? GetNavalDlcModelType()
        {
            // Return cached result if type check already completed
            if (_typeCheckCompleted)
            {
            ...
            {
                // Mark type check as completed (whether successful or not)
                _typeCheckCompleted = true;
            }
