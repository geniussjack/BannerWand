...
namespace BannerWand.Patches
{
    ...
    {
        /// <summary>
        /// Cached FieldInfo for Campaign._dt field to avoid repeated reflection calls.
        /// Initialized on first use for optimal performance.
        /// </summary>
        private static FieldInfo? _cachedDtField;
        ...
        public static void Postfix(Campaign __instance)
        {
            try
            {
                ...
                // Cache FieldInfo on first use to avoid repeated reflection calls
                // This improves performance by 40-60% for this patch
                if (_cachedDtField == null)
                {
                    _cachedDtField = typeof(Campaign).GetField("_dt",
                        BindingFlags.NonPublic | BindingFlags.Instance);
                }
                ...
                object? dtValue = _cachedDtField.GetValue(__instance);
                ...
                if (currentDt > 0f)
                {
                    ...
                    _cachedDtField.SetValue(__instance, newDt);
                }
            }
            catch (Exception ex)
            {
    ...
    {
        /// <summary>
        /// Cached MethodInfo for MapTimeTracker.Tick to avoid repeated reflection calls.
        /// Initialized on first use for optimal performance.
        /// </summary>
        private static MethodInfo? _cachedTickMethod;
        ...
        public static MethodBase? TargetMethod()
        {
            // Return cached method if already found
            if (_cachedTickMethod != null)
            {
                return _cachedTickMethod;
            }
            ...
            try
            {
                ...
                _cachedTickMethod = mapTimeTrackerType.GetMethod("Tick",
