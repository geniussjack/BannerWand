            ...
            try
            {
                ...
                if (Settings.MilitiaVeteranChance > GameConstants.FloatEpsilon)
                {
                    if (SettlementCheatHelper.ShouldApplyCheatToSettlement(settlement))
                    {
                        ...
                        ModLogger.DebugThrottled($"[CustomSettlementMilitiaModel] Applied veteran chance {Settings.MilitiaVeteranChance:F0}% to {settlement.Name}: {originalChance:F3} -> {baseChance.ResultNumber:F3}");
                    }
                }
