    ...
    public class CustomGenericXpModel : GenericXpModel
    {
        ...
        public override float GetXpMultiplier(Hero hero)
        {
            try
            {
                ...
                if (!shouldApply)
                {
                    // Debug logging for clan members (throttled to avoid spam)
                    if (hero.Clan == Clan.PlayerClan && hero != Hero.MainHero)
                    {
                        ModLogger.DebugThrottled($"[CustomGenericXpModel] Hero {hero.Name} is in player clan but ShouldApplyCheat returned false. ApplyToPlayerClanMembers={TargetSettings.ApplyToPlayerClanMembers}");
                    }
                    return DefaultMultiplier;
                }
                ...
                {
                    ModLogger.DebugThrottled($"[CustomGenericXpModel] Applying {multiplier}x multiplier to clan member {hero.Name}");
                }
