        ...
        public override ExplainedNumber GetTotalWage(MobileParty mobileParty, TroopRoster troopRoster, bool includeDescriptions = false)
        {
            try
            {
                ...
                if (Math.Abs(multiplier) < GameConstants.FloatEpsilon)
                {
                    ...
                    DebugThrottled($"[CustomPartyWageModel] Set garrison wages to 0 (free) for {settlement.Name}: {originalWage:F2} -> 0");
                    return freeWage;
                }
                ...
                if (multiplier > GameConstants.MultiplierFactorBase)
                {
                    ...
                    DebugThrottled($"[CustomPartyWageModel] Applied multiplier {multiplier:F2} to garrison wages for {settlement.Name}: {originalWage:F2} -> {newWage:F2}");
